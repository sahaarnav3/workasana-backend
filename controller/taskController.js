...
module.exports.fetchTasks = async (req, res) => {
  try {
    ...
    if(req.query.project)
        searchQuery.project = req.query.project;
    if(req.query.team)
        searchQuery.team = req.query.team;
    ...
    const taskResponse = await Task.find(searchQuery)
      ...
      if(!taskResponse || taskResponse.length < 1)
        return res.status(400).json({ error: "No task with given filter value(s) found. Please try again." });
